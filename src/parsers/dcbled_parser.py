...
def extract_report_metadata(text: str, file_name: str) -> Report:
    # get collection date
    match = re.search(r"Datum obravnave:\s*(\d{2}\.\d{2}\.\d{4})", text)
    if match:
...
def extract_celiac_markers(results_text: str):
...
def parse_pdf(file_path: Path) -> tuple[Report, list[TestResult]]:
    ...
    results_text = extract_results_text(text)
    all_results = [extract_test_result(m) for m in pattern.finditer(results_text)]
    all_results.extend(extract_celiac_markers(results_text))
